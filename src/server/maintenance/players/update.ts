...
import { PlayerId, Projectile, TeamId, Player } from '../../../types';
...
export default class GamePlayersUpdate extends System {
  ...
  constructor({ app }) {
    ...
    this.afkDisconnectMs = this.app.config.afkDisconnectTimeout * SECONDS_PER_MINUTE * MS_PER_SEC;
  ...
  onUpdatePlayers(frame: number, frameFactor: number): void {
    ...
    while (player !== undefined) {
       ...
       */
      if (!isConnected || player.alivestatus.current !== PLAYERS_ALIVE_STATUSES.ALIVE) {
  ...
  // private updatePredator(player: Player): void {}

  // private updateGoliath(player: Player): void {}

  // private updateCopter(player: Player): void {}

  // private updateTornado(player: Player): void {}

  // private updateProwler(player: Player): void {}
}
