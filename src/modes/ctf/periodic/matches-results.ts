...
export default class MatchesResultsPeriodic extends System {
  ...
  protected reduceSamplesArray(values: number[], playerId: PlayerId): void {
    if (this.storage.playerList.has(playerId) && values.length > 239) {
      values.sort((a, b) => a - b);
  ...
  protected storePlayerStats(player: Player, disconnectTime = 0): void {
  ...
  protected saveStatsToFile(): void {
    ...
    this.playersStats.forEach(playerStat => {
      if (this.ping.has(playerStat.id)) {
        // eslint-disable-next-line no-param-reassign
        playerStat.p = median(this.ping.get(playerStat.id));
      }

      if (this.distanceToOwnBase.has(playerStat.id)) {
        // eslint-disable-next-line no-param-reassign
        playerStat.bo = median(this.distanceToOwnBase.get(playerStat.id));
      }

      if (this.distanceToEnemyBase.has(playerStat.id)) {
        // eslint-disable-next-line no-param-reassign
    ...
    try {
      ...
      this.log.error('Error while serialising players stats: %o', { error: err.stack });
    }

    writeFile(joinPath(this.saveDir, fileName), content, err => {
      if (err) {
        this.log.error('Error while saving players stats: %o', { error: err.stack });
      }
    });
      ...
      readdirSync(this.saveDir).forEach(file => {
        ...
        if (parseInt(name, 10) < outdatedAt) {
          try {
            ...
            this.log.error('Error while file deleting: %o', { file, error: err.stack });
          }
        }
      });
    } catch (err) {
      this.log.error('Error while dir reading: %o', { dir: this.saveDir, error: err.stack });
    }
  }
  ...
  storePlayersMetricSample(): void {
    this.storage.playerList.forEach(player => {
      ...
      if (
        !this.storage.botIdList.has(playerId) &&
  ...
  onSecondTick(): void {
    if (this.shouldSaveStats && this.seconds === 2) {
      this.storePlayersStats();
    } else if (this.shouldSaveStats && this.seconds === this.fileSaveDelaySec) {
